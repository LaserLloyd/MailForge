...
- **Retention policy.** Spam and scam mail is deleted immediately; everything
  ...
  provider. A background sweep runs every six hours; `emailforge
...
### Added
- **Demo mode** — `emailforge demo` builds a throwaway application home,
...
- `EMAILFORGE_HOME` relocates config and data together; `run_serve()` and
