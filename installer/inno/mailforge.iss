; Inno Setup script — MailForge Windows installer (build spec §10).
;
; Flow:
;   1. PyInstaller folder-mode build produces dist\mailforge\ (see
;      packaging/pyinstaller.spec — NEVER onefile).
;   2. This script packages that folder into a single Setup.exe.
;   3. On install it lays the folder under {autopf}\MailForge and
...
#define MyAppName "MailForge"
#define MyAppVersion "0.1.0"
#define MyAppPublisher "MailForge"
#define MyAppExeName "mailforge.exe"
#define ServiceId "mailforge"

[Setup]
AppId={{B7E1F0A2-0C3D-4E5F-9A8B-MAILFORGEAPP}}
AppName={#MyAppName}
...
OutputBaseFilename=mailforge-setup
...
Source: "..\..\dist\mailforge\*"; DestDir: "{app}"; Flags: recursesubdirs createallsubdirs
