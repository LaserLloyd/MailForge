...
wrapped by Inno Setup (see installer/inno/openclaw-email.iss), and the
...
Output: dist/openclaw-email/  (the folder Inno Setup packages).
"""
...
datas = []
datas += [(str(SRC / "db" / "schema.sql"), "openclaw_email/db")]
datas += [(str(SRC / "llm" / "prompts"), "openclaw_email/llm/prompts")]
...
hiddenimports = []
hiddenimports += collect_submodules("openclaw_email")
