...
wrapped by Inno Setup (see installer/inno/mailforge.iss), and the
...
Output: dist/mailforge/  (the folder Inno Setup packages).
"""
...
datas = []
datas += [(str(SRC / "db" / "schema.sql"), "mailforge/db")]
datas += [(str(SRC / "llm" / "prompts"), "mailforge/llm/prompts")]
...
hiddenimports = []
hiddenimports += collect_submodules("mailforge")
