...
silently does nothing:
  ...
  * the OS keyring — ``bluebox.imap.<name>`` for the listener AND
    ``bluebox.smtp.<username>`` for the send path. The two are keyed
  ...
  * the ``accounts`` DB row — what the Settings "Inbox Coverage" panel reads.
    :class:`~bluebox.mail.imap_listener.IMAPListener` upserts this itself
