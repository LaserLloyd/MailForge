...
Writes ``~/.config/systemd/user/bluebox.service`` from the vendored
template (``installer/systemd/bluebox.service.tmpl``) with the spec's
...
def _resolve_exec_start() -> str:
    """Absolute ``ExecStart`` command for ``bluebox serve`` (spec §10).

    Prefer the installed ``bluebox`` console script; if it is not on PATH
    ...
    return f"{py} -m bluebox.cli serve"
...
def _template() -> str | None:
    ...
    """
    # package is src/bluebox/service -> repo root is three parents up.
    repo_tmpl = (
        Path(__file__).resolve().parents[3]
        / "installer"
        / "systemd"
        / "bluebox.service.tmpl"
    )
...
def install(unit_dir: Path | None = None) -> str:
    ...
    if not have_systemctl:
        print(
            ...
            "  systemctl --user enable --now bluebox"
        )
        return str(unit_path)

    _run(["loginctl", "enable-linger", user])
    _run(["systemctl", "--user", "daemon-reload"])
    if _run(["systemctl", "--user", "enable", "--now", "bluebox"]):
        log.info("bluebox user service enabled and started")
    else:
        print(
...
def start() -> None:
    ...
    if not _run(["systemctl", "--user", "start", "bluebox"]):
        print("Could not start service. Try: systemctl --user start bluebox")


def stop() -> None:
    """Stop the user service (spec §10)."""
    if not _run(["systemctl", "--user", "stop", "bluebox"]):
        print("Could not stop service. Try: systemctl --user stop bluebox")


# Fallback template if the installer/ tree was not packaged with the wheel.
_INLINE_TEMPLATE = """[Unit]
Description=BlueBox (local-LLM email triage & auto-draft, no auto-send)
