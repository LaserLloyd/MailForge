...
Wraps ``uv tool run bluebox serve`` with **WinSW-x64.exe** (MIT) — NOT
...
XML_NAME = "bluebox.xml"

# WinSW descriptor. WinSW derives the .xml path from the renamed exe, but we
# keep an explicit, conventional pair (bluebox.exe + bluebox.xml)
...
_XML_TEMPLATE = """<service>
  ...
  <arguments>tool run bluebox serve</arguments>
