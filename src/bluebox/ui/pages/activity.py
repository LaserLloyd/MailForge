...
def _verify_banner(store: object) -> None:
    ...
    with ui.row().classes("w-full items-center bb-card q-pa-md").style("gap: 10px"):
        if ok:
...
def _event_list(store: object) -> None:
    ...
    with ui.column().classes("w-full").style("gap: 6px"):
        for r in shown:
            with ui.element("div").classes("bb-row w-full").style("cursor: default"):
                with ui.row().classes("w-full items-center no-wrap").style("gap: 10px"):
                    ...
                    if subject:
                        ui.label(subject).classes("bb-mono").style(
                            "font-size: 12px; color: var(--text-secondary)"
                        )
                    detail = (r["detail_json"] or "")[:140]
                    ui.label(detail).classes("bb-mono col").style(
