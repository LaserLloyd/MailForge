...
#: ``<home>/data``). Used by ``emailforge demo`` to run against a throwaway
#: tree, and handy for testing a second profile without touching the real one.
HOME_ENV = "EMAILFORGE_HOME"
...
def app_home() -> Path | None:
    """Explicit application home from ``EMAILFORGE_HOME``, if set."""
...
def prompt_override_dir() -> Path:
    ...
    here; :func:`emailforge.llm.prompts.load` prefers them over the
    packaged defaults. Absent/empty => packaged defaults are used, so the app
    is fully standalone. Overridable via ``EMAILFORGE_PROMPT_DIR``.
    """
    env = os.environ.get("EMAILFORGE_PROMPT_DIR")
    if env:
        return _expand(env)
...
def desktop_entry_path() -> Path:
    """``~/.local/share/applications/emailforge.desktop`` (XDG app drawer)."""
    base = os.environ.get("XDG_DATA_HOME") or os.path.expanduser("~/.local/share")
    return Path(base) / "applications" / "emailforge.desktop"


def icon_path() -> Path:
    """Installed app icon location (hicolor scalable theme)."""
    base = os.environ.get("XDG_DATA_HOME") or os.path.expanduser("~/.local/share")
    return Path(base) / "icons" / "hicolor" / "scalable" / "apps" / "emailforge.svg"
