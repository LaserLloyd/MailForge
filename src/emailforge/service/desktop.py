...
Puts EmailForge in the app drawer/launcher. Clicking it runs
``emailforge open``, which makes sure the background service is up and then
...
def _exec_cmd() -> str:
    """``emailforge open`` using the absolute console-script path if known."""
    exe = shutil.which("emailforge")
    return f"{exe} open" if exe else "emailforge open"
