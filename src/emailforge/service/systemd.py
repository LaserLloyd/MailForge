...
Writes ``~/.config/systemd/user/emailforge.service`` from the vendored
template (``installer/systemd/emailforge.service.tmpl``) with the spec's
...
def _resolve_exec_start() -> str:
    """Absolute ``ExecStart`` command for ``emailforge serve`` (spec §10).

    Prefer the installed ``emailforge`` console script; if it is not on PATH
    (e.g. a dev checkout), fall back to the current interpreter running the
    module entrypoint so the unit is always valid.
    """
    exe = shutil.which("emailforge")
    if exe:
        return f"{exe} serve"
    py = shutil.which("python3") or shutil.which("python") or "python3"
    return f"{py} -m emailforge.cli serve"
...
def _template() -> str | None:
    ...
    """
    # package is src/emailforge/service -> repo root is three parents up.
    repo_tmpl = (
        Path(__file__).resolve().parents[3]
        / "installer"
        / "systemd"
        / "emailforge.service.tmpl"
    )
...
def install(unit_dir: Path | None = None) -> str:
    ...
    if not have_systemctl:
        print(
            ...
            "  systemctl --user enable --now emailforge"
        )
        return str(unit_path)

    _run(["loginctl", "enable-linger", user])
    _run(["systemctl", "--user", "daemon-reload"])
    if _run(["systemctl", "--user", "enable", "--now", "emailforge"]):
        log.info("emailforge user service enabled and started")
    else:
        print(
...
def start() -> None:
    ...
    if not _run(["systemctl", "--user", "start", "emailforge"]):
        print("Could not start service. Try: systemctl --user start emailforge")


def stop() -> None:
    """Stop the user service (spec §10)."""
    if not _run(["systemctl", "--user", "stop", "emailforge"]):
        print("Could not stop service. Try: systemctl --user stop emailforge")


# Fallback template if the installer/ tree was not packaged with the wheel.
_INLINE_TEMPLATE = """[Unit]
Description=EmailForge (local-LLM email triage & auto-draft, no auto-send)
