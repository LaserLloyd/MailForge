...
def _render_row(
    ...
    classes = "bb-row w-full"
    if selected_id == message_id:
        classes += " bb-row--selected"
    with (
        ui.element("div")
        .classes(classes)
    ...
    with ui.card().classes("w-full bb-card").style("padding: 12px; gap: 6px"):
        ...
        for att in rows:
            ...
            with ui.row().classes("w-full items-center no-wrap").style("gap: 8px"):
                ...
                ui.label(str(_value(att, "filename", "attachment"))).classes("bb-clip-1").style(
...
def _message_reader(
    ...
    *,
    ...
    with (
        ...
        ui.card().classes("bb-card").style("width: min(560px, 94vw); padding: 18px"),
    ):
    ...
    with ui.row().classes("w-full items-center bb-toolbar").style("gap: 6px"):
        if full_page:
    ...
    if quarantined:
        with (
            ui.card().classes("w-full bb-card").style("padding: 12px; border-color: var(--error)")
        ):
    ...
    with ui.card().classes("w-full bb-card").style("padding: 10px; gap: 4px"):
        with ui.row().classes("items-center").style("gap: 7px"):
    ...
    with ui.card().classes("w-full bb-card").style("padding: 12px; gap: 3px"):
        ui.label(
            f"From: {sender}"
            + (
    ...
    if working is not None:
        with ui.card().classes("w-full bb-card").style("padding: 11px"):
    ...
    with ui.row().classes("w-full bb-sticky-actions bb-toolbar"):
...
def render(
    ...
    *,
    ...
    with (
        ui.row()
        .classes("w-full items-center bb-card bb-toolbar")
    ...
    workspace_classes = "bb-mail-workspace bb-card w-full"
    if message_id is not None:
        workspace_classes += " has-selection"
    with ui.element("div").classes(workspace_classes):
        with ui.column().classes("bb-mail-list").style("gap: 7px; padding: 10px"):
            if not rows:
            ...
            if len(rows) >= page_size or page:
                with ui.row().classes("w-full items-center justify-between").style(
                    ...
                    if len(rows) < page_size:
                        older.props("disable")
        with ui.column().classes("bb-mail-pane").style("gap: 12px"):
            if message_id is None:
                ...
                with ui.row().classes("w-full bb-menu-btn"):
                    ui.button(
                        "Back to inbox",
                        icon="arrow_back",
                        on_click=lambda: _go(),
