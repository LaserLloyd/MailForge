...
.bb-header-note { white-space: nowrap; }
.bb-toolbar { gap: 12px; }
.bb-ai-drawer {
  background: var(--bg-secondary) !important;
  border-left: 1px solid var(--border);
}
.bb-chat-scroll { height: calc(100vh - 260px); overflow-y: auto; }
.bb-chat-message {
...
.bb-chat-message--user { background: var(--user-bubble); }
.bb-chat-message--assistant { background: var(--bot-bubble); }
...
.bb-nav-btn:hover { background: var(--bg-hover); color: var(--text-primary); }
.bb-nav-btn--active {
...
.bb-brand-name { font-weight: 800; letter-spacing: -0.02em; }
.bb-nav-btn .q-icon { margin-right: 8px; }
.bb-nav-btn .q-btn__content { width: 100%; justify-content: flex-start; }
.bb-main-column { min-width: 0; }
.bb-mail-workspace {
  display: grid;
...
.bb-mail-list > * { flex: 0 0 auto; }
.bb-mail-pane {
  overflow-y: auto;
  max-height: calc(100vh - 160px);
  padding: 16px 20px 96px; /* room for the sticky action bar */
}
.bb-row--selected { border-color: var(--accent); background: var(--accent-muted); }
.bb-reader-window {
  background: var(--bg-primary) !important;
  color: var(--text-primary);
  padding: 0 !important;
}
.bb-reader-window-header {
  position: sticky; top: 0; z-index: 2;
  background: var(--bg-secondary);
  border-bottom: 1px solid var(--border);
}
.bb-reader-window-content { max-width: 920px; margin: 0 auto; }
.bb-markdown { overflow-wrap: anywhere; line-height: 1.62; }
.bb-markdown h1, .bb-markdown h2, .bb-markdown h3 { margin: 1em 0 .45em; }
/* email headings stay email-sized (resets removed the browser scale) */
.bb-markdown h1 { font-size: 1.45rem; font-weight: 800; }
.bb-markdown h2 { font-size: 1.25rem; font-weight: 700; }
.bb-markdown h3 { font-size: 1.1rem;  font-weight: 700; }
.bb-markdown ul { list-style: disc; padding-left: 1.5em; margin: .5em 0; }
.bb-markdown ol { list-style: decimal; padding-left: 1.5em; margin: .5em 0; }
.bb-markdown blockquote {
  border-left: 3px solid var(--border-light);
  padding-left: 12px; margin: .6em 0;
  color: var(--text-secondary);
}
.bb-markdown p { margin: .6em 0; }
.bb-markdown table { border-collapse: collapse; max-width: 100%; overflow-x: auto; }
.bb-markdown th, .bb-markdown td { border: 1px solid var(--border); padding: 7px 9px; }
.bb-sticky-actions {
...
.bb-card .q-card__section { padding: 14px 16px; }
...
.bb-row:hover { background: var(--bg-hover); border-color: var(--border-light); }
.bb-row .col, .bb-row .q-column { min-width: 0; }
.bb-row .q-label { max-width: 100%; }
...
.bb-subject--full { white-space: normal; overflow-wrap: anywhere; }
.bb-clip-1 {
...
.bb-bubble--bot { background: var(--bot-bubble); border-bottom-left-radius: 4px; }
.bb-bubble--user {
...
.bb-badge--accent  { background: var(--accent-muted); border-color: var(--accent); color: var(--accent-hover); }
.bb-badge--success { background: rgba(52,211,153,0.12); border-color: var(--success); color: var(--success); }
.bb-badge--error   { background: rgba(248,113,113,0.12); border-color: var(--error); color: var(--error); }
.bb-badge--warning { background: rgba(251,191,36,0.12);  border-color: var(--warning); color: var(--warning); }
...
.bb-stat .bb-stat-n { font-size: 22px; font-weight: 700; line-height: 1.1; }
.bb-stat .bb-stat-l { font-size: 11.5px; color: var(--text-secondary); text-transform: uppercase; letter-spacing: 0.06em; }

/* per-inbox tiles (dashboard) — header + grid of clickable metrics */
.bb-inbox-tile { flex: 1 1 320px; min-width: 0; padding: 14px; gap: 11px; }
.bb-metric-grid {
  display: grid;
...
.bb-metric:hover { background: var(--bg-hover); border-color: var(--accent); }
.bb-metric .bb-metric-n { font-size: 19px; font-weight: 700; line-height: 1.15; }
...
.bb-metric--zero { opacity: 0.66; }
.bb-metric--zero .bb-metric-n { color: var(--text-muted); }

/* health dot */
.bb-dot { width: 11px; height: 11px; border-radius: 50%; background: var(--text-muted); }
.bb-dot--up   { background: var(--success); box-shadow: 0 0 7px rgba(52,211,153,0.7); }
.bb-dot--down { background: var(--error);   box-shadow: 0 0 7px rgba(248,113,113,0.7); }
.bb-dot--warn { background: var(--warning); box-shadow: 0 0 7px rgba(251,191,36,0.7); }
.bb-dot--busy { background: var(--accent-hover); animation: bb-pulse 1s ease-in-out infinite; }
@keyframes bb-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.35; } }

/* dashboard bulk-action strip: quiet until something is ticked */
.bb-bulkbar { transition: border-color 0.12s ease; }
.bb-bulkbar--active { border-color: var(--accent) !important; }
...
.bb-sync .bb-sync-text {
  font-size: 11.5px; color: var(--text-secondary); white-space: nowrap;
  max-width: 260px; overflow: hidden; text-overflow: ellipsis;
}
.bb-sync--error .bb-sync-text { color: var(--error); font-weight: 600; }
.bb-sync--warn  .bb-sync-text { color: var(--warning); font-weight: 600; }
.bb-sync--new { cursor: pointer; border-color: var(--accent); }
.bb-sync--new .bb-sync-text { color: var(--accent-hover); font-weight: 700; }
@media (max-width: 760px) { .bb-sync .bb-sync-text { display: none; } }
...
code, pre, .q-code, .bb-mono { font-family: var(--font-mono); }
...
@media (max-width: 760px) {
  ...
  .bb-header-note { display: none; }
  .bb-toolbar { align-items: stretch !important; flex-wrap: wrap !important; }
  .bb-toolbar .bb-toolbar-grow { width: 100% !important; max-width: none !important; }
  .bb-toolbar .bb-toolbar-grow.col { flex: 1 1 100% !important; }
  .bb-toolbar .q-tabs { width: 100% !important; max-width: 100% !important; overflow: hidden; }
  .bb-toolbar .q-tabs__content {
    justify-content: flex-start !important;
    overflow-x: auto !important;
  }
  .bb-stat { min-width: 0; flex: 1 1 120px; }
  .bb-row { padding: 11px 12px; }
  .bb-ai-drawer { width: min(94vw, 440px) !important; }
  .bb-mail-workspace { display: block; min-height: 0; }
  .bb-mail-list, .bb-mail-pane { max-height: none; border-right: 0; }
  .bb-mail-workspace.has-selection .bb-mail-list { display: none; }
  .bb-mail-workspace:not(.has-selection) .bb-mail-pane { display: none; }
}
"""
...
def badge(text: str, kind: str = "") -> ui.html:
    ...
    cls = f"bb-badge bb-badge--{kind}" if kind else "bb-badge"
...
def subject_label(subject: Any, *, full: bool = False, style: str = "") -> ui.label:
    ...
    label = ui.label(text).classes("bb-subject--full" if full else "bb-subject")
    if style:
        label.style(style)
    if not full:
        with label:
...
def _health_dot(bridge: object | None, store: object | None = None) -> None:
    ...
    dot = ui.element("div").classes("bb-dot")
    with dot:
    ...
    async def _probe() -> None:
        ...
        try:
            ...
            up = False
        try:
            dot.classes(add="bb-dot--up" if up else "bb-dot--down")
...
def _sync_widget(store: object | None = None) -> None:
    ...
    def _describe() -> tuple[str, str, str]:
        ...
        accounts = s["accounts"]
        if s["state"] == "none":
            return "bb-dot--down", "No mailboxes", "No IMAP account listener is running."
        lines = []
        for a in accounts:
            if a.last_error:
        ...
        if s["checking"]:
            return "bb-dot--busy", "Checking mail…", tip
        if s["state"] == "error":
            first = s["errors"][0]
            return "bb-dot--down", f"Mail sync down: {first.last_error}", tip
        if s["state"] == "degraded":
            names = ", ".join(e.account for e in s["errors"])
            return "bb-dot--warn", f"Sync problem: {names}", tip
        if s["state"] == "starting":
            return "bb-dot", "Connecting to mail…", tip
        return "bb-dot--up", f"Mail checked {human_age(s['age'])}", tip
    ...
    chip = ui.element("div").classes("bb-sync")
    with chip:
        dot = ui.element("div").classes(f"bb-dot {kind}")
        label = ui.label(text).classes("bb-sync-text")
        tooltip = ui.tooltip(tip).style("white-space: pre-line; max-width: 420px")
        btn = ui.button(icon="refresh").props("flat round dense size=sm").classes("bb-sync-btn")
    ...
    _dot_kinds = ("bb-dot--up", "bb-dot--down", "bb-dot--warn", "bb-dot--busy")

    def _paint() -> None:
        kind, text, tip = _describe()
        gained = sum(a.new_total for a in registry.snapshot()) - state["baseline"]
        state["has_new"] = max(0, gained)
        if state["has_new"] and kind == "bb-dot--up":
            n = state["has_new"]
        ...
        chip.classes(
            remove="bb-sync--error bb-sync--warn bb-sync--new",
            add=(
                "bb-sync--error" if kind == "bb-dot--down" and registry.summary()["state"] != "none"
                else "bb-sync--warn" if kind == "bb-dot--warn"
                else "bb-sync--new" if state["has_new"] else ""
            ),
        )
...
def shell(
    ...
    with drawer:
        with ui.column().classes("w-full q-pa-sm").style("gap: 10px"):
            with ui.row().classes("items-center no-wrap q-px-sm").style("gap: 10px"):
                with ui.element("div").classes("bb-logo"):
                    ui.html("&#9993;")  # envelope glyph
                ui.label("EmailForge").classes("bb-brand-name")
            ...
            for key, label, icon, route in NAV:
                ...
                if key == active:
                    btn.classes(add="bb-nav-btn--active")

    with ui.header(elevated=False).classes("bb-header items-center q-px-md q-py-sm"):
    ...
    with ui.column().classes("w-full mx-auto bb-main-column").style(
