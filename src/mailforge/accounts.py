...
silently does nothing:
  ...
  * the OS keyring — ``mailforge.imap.<name>`` for the listener AND
    ``mailforge.smtp.<username>`` for the send path. The two are keyed
  ...
  * the ``accounts`` DB row — what the Settings "Inbox Coverage" panel reads.
    :class:`~mailforge.mail.imap_listener.IMAPListener` upserts this itself
