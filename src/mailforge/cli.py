...
def version() -> None:
    """Print version."""
    typer.echo(f"mailforge {__version__}")
...
def setup_wizard() -> None:
    ...
    typer.secho(
        "\nDone. Start the agent with:  mailforge serve\n"
...
def account_add(
    ...
    typer.secho("Restart mailforge.service for the listener to take effect.", fg="yellow")
...
def account_list() -> None:
    ...
    if not settings.imap_accounts:
        typer.secho("No mailboxes configured. Add one with: mailforge account-add", fg="yellow")
        return
...
def account_remove(
    ...
    typer.secho("Restart mailforge.service to stop the listener.", fg="yellow")
...
def demo(
    ...
    home = Path(tempfile.mkdtemp(prefix="mailforge-demo-"))
    ...
    os.environ["MAILFORGE_HOME"] = str(home)
    ...
    os.environ["MAILFORGE_NO_LISTENERS"] = "1"
...
def open(  # noqa: A001 - intentional CLI verb
    ...
    if not _up():
        typer.secho(
            "UI is not running. Start it with 'mailforge service start' "
            "or 'mailforge serve', then try again.",
            fg="red",
        )
        raise typer.Exit(code=1)
