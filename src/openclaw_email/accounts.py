...
silently does nothing:
  ...
  * the OS keyring — ``openclaw-email.imap.<name>`` for the listener AND
    ``openclaw-email.smtp.<username>`` for the send path. The two are keyed
  ...
  * the ``accounts`` DB row — what the Settings "Inbox Coverage" panel reads.
    :class:`~openclaw_email.mail.imap_listener.IMAPListener` upserts this itself
