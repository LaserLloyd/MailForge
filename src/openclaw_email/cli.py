...
def account_add(
    ...
    typer.secho("Restart openclaw-email.service for the listener to take effect.", fg="yellow")
...
def account_list() -> None:
    ...
    if not settings.imap_accounts:
        typer.secho("No mailboxes configured. Add one with: openclaw-email account-add", fg="yellow")
        return
...
def account_remove(
    ...
    typer.secho("Restart openclaw-email.service to stop the listener.", fg="yellow")
...
def demo(
    ...
    home = Path(tempfile.mkdtemp(prefix="openclaw-email-demo-"))
    ...
    os.environ["OPENCLAW_EMAIL_HOME"] = str(home)
    ...
    os.environ["OPENCLAW_EMAIL_NO_LISTENERS"] = "1"
