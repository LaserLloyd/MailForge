...
Puts OpenClaw Email in the app drawer/launcher. Clicking it runs
``openclaw-email open``, which makes sure the background service is up and then
...
def _exec_cmd() -> str:
    """``openclaw-email open`` using the absolute console-script path if known."""
    exe = shutil.which("openclaw-email")
    return f"{exe} open" if exe else "openclaw-email open"
