...
Writes ``~/.config/systemd/user/openclaw-email.service`` from the vendored
template (``installer/systemd/openclaw-email.service.tmpl``) with the spec's
...
UNIT_NAME = "openclaw-email.service"
...
def _resolve_exec_start() -> str:
    """Absolute ``ExecStart`` command for ``openclaw-email serve`` (spec §10).

    Prefer the installed ``openclaw-email`` console script; if it is not on PATH
    (e.g. a dev checkout), fall back to the current interpreter running the
    module entrypoint so the unit is always valid.
    """
    exe = shutil.which("openclaw-email")
    if exe:
        return f"{exe} serve"
    py = shutil.which("python3") or shutil.which("python") or "python3"
    return f"{py} -m openclaw_email.cli serve"
...
def _template() -> str | None:
    ...
    """
    # package is src/openclaw_email/service -> repo root is three parents up.
    repo_tmpl = (
        Path(__file__).resolve().parents[3]
        / "installer"
        / "systemd"
        / "openclaw-email.service.tmpl"
    )
...
def install(unit_dir: Path | None = None) -> str:
    ...
    if not have_systemctl:
        print(
            ...
            "  systemctl --user enable --now openclaw-email"
        )
        return str(unit_path)

    _run(["loginctl", "enable-linger", user])
    _run(["systemctl", "--user", "daemon-reload"])
    if _run(["systemctl", "--user", "enable", "--now", "openclaw-email"]):
        log.info("openclaw-email user service enabled and started")
    else:
        print(
...
def start() -> None:
    ...
    if not _run(["systemctl", "--user", "start", "openclaw-email"]):
        print("Could not start service. Try: systemctl --user start openclaw-email")


def stop() -> None:
    """Stop the user service (spec §10)."""
    if not _run(["systemctl", "--user", "stop", "openclaw-email"]):
        print("Could not stop service. Try: systemctl --user stop openclaw-email")


# Fallback template if the installer/ tree was not packaged with the wheel.
_INLINE_TEMPLATE = """[Unit]
Description=OpenClaw Email Agent (local-LLM email triage & auto-draft, no auto-send)
