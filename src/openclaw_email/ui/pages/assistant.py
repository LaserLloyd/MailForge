...
def render_drawer(
    ...
    *,
    ...
    with drawer:
        ...
        chat = ui.column().classes("w-full q-px-sm oce-chat-scroll").style("gap: 10px")
                    ...
                    with ui.column().classes(
                        ...
                        if role == "assistant":
                            with ui.element("div").classes(
                                "oce-chat-message oce-chat-message--assistant"
                            ):
                                safe_markdown(text)
                        else:
                            ui.label(text).classes(
                                "oce-chat-message oce-chat-message--user"
                            )
                        if role == "assistant":
