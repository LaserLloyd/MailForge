...
silently does nothing:
  ...
  * the OS keyring — ``siftforge.imap.<name>`` for the listener AND
    ``siftforge.smtp.<username>`` for the send path. The two are keyed
  ...
  * the ``accounts`` DB row — what the Settings "Inbox Coverage" panel reads.
    :class:`~siftforge.mail.imap_listener.IMAPListener` upserts this itself
