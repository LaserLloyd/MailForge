...
def version() -> None:
    """Print version."""
    typer.echo(f"siftforge {__version__}")
...
def setup_wizard() -> None:
    ...
    typer.secho(
        "\nDone. Start the agent with:  siftforge serve\n"
...
def account_add(
    ...
    typer.secho("Restart siftforge.service for the listener to take effect.", fg="yellow")
...
def account_list() -> None:
    ...
    if not settings.imap_accounts:
        typer.secho("No mailboxes configured. Add one with: siftforge account-add", fg="yellow")
        return
...
def account_remove(
    ...
    typer.secho("Restart siftforge.service to stop the listener.", fg="yellow")
...
def demo(
    ...
    home = Path(tempfile.mkdtemp(prefix="siftforge-demo-"))
    ...
    os.environ["SIFTFORGE_HOME"] = str(home)
    ...
    os.environ["SIFTFORGE_NO_LISTENERS"] = "1"
...
def open(  # noqa: A001 - intentional CLI verb
    ...
    if not _up():
        typer.secho(
            "UI is not running. Start it with 'siftforge service start' "
            "or 'siftforge serve', then try again.",
            fg="red",
        )
        raise typer.Exit(code=1)
