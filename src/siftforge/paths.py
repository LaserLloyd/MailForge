...
#: ``<home>/data``). Used by ``siftforge demo`` to run against a throwaway
#: tree, and handy for testing a second profile without touching the real one.
HOME_ENV = "SIFTFORGE_HOME"
...
def app_home() -> Path | None:
    """Explicit application home from ``SIFTFORGE_HOME``, if set."""
...
def prompt_override_dir() -> Path:
    ...
    here; :func:`siftforge.llm.prompts.load` prefers them over the
    packaged defaults. Absent/empty => packaged defaults are used, so the app
    is fully standalone. Overridable via ``SIFTFORGE_PROMPT_DIR``.
    """
    env = os.environ.get("SIFTFORGE_PROMPT_DIR")
    if env:
        return _expand(env)
...
def desktop_entry_path() -> Path:
    """``~/.local/share/applications/siftforge.desktop`` (XDG app drawer)."""
    base = os.environ.get("XDG_DATA_HOME") or os.path.expanduser("~/.local/share")
    return Path(base) / "applications" / "siftforge.desktop"


def icon_path() -> Path:
    """Installed app icon location (hicolor scalable theme)."""
    base = os.environ.get("XDG_DATA_HOME") or os.path.expanduser("~/.local/share")
    return Path(base) / "icons" / "hicolor" / "scalable" / "apps" / "siftforge.svg"
