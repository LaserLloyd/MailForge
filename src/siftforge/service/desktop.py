...
Puts SiftForge in the app drawer/launcher. Clicking it runs
``siftforge open``, which makes sure the background service is up and then
...
def _exec_cmd() -> str:
    """``siftforge open`` using the absolute console-script path if known."""
    exe = shutil.which("siftforge")
    return f"{exe} open" if exe else "siftforge open"
