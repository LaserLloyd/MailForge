...
Writes ``~/.config/systemd/user/siftforge.service`` from the vendored
template (``installer/systemd/siftforge.service.tmpl``) with the spec's
...
def _resolve_exec_start() -> str:
    """Absolute ``ExecStart`` command for ``siftforge serve`` (spec §10).

    Prefer the installed ``siftforge`` console script; if it is not on PATH
    (e.g. a dev checkout), fall back to the current interpreter running the
    module entrypoint so the unit is always valid.
    """
    exe = shutil.which("siftforge")
    if exe:
        return f"{exe} serve"
    py = shutil.which("python3") or shutil.which("python") or "python3"
    return f"{py} -m siftforge.cli serve"
...
def _template() -> str | None:
    ...
    """
    # package is src/siftforge/service -> repo root is three parents up.
    repo_tmpl = (
        Path(__file__).resolve().parents[3]
        / "installer"
        / "systemd"
        / "siftforge.service.tmpl"
    )
...
def install(unit_dir: Path | None = None) -> str:
    ...
    if not have_systemctl:
        print(
            ...
            "  systemctl --user enable --now siftforge"
        )
        return str(unit_path)

    _run(["loginctl", "enable-linger", user])
    _run(["systemctl", "--user", "daemon-reload"])
    if _run(["systemctl", "--user", "enable", "--now", "siftforge"]):
        log.info("siftforge user service enabled and started")
    else:
        print(
...
def start() -> None:
    ...
    if not _run(["systemctl", "--user", "start", "siftforge"]):
        print("Could not start service. Try: systemctl --user start siftforge")


def stop() -> None:
    """Stop the user service (spec §10)."""
    if not _run(["systemctl", "--user", "stop", "siftforge"]):
        print("Could not stop service. Try: systemctl --user stop siftforge")


# Fallback template if the installer/ tree was not packaged with the wheel.
_INLINE_TEMPLATE = """[Unit]
Description=SiftForge (local-LLM email triage & auto-draft, no auto-send)
