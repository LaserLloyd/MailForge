...
from emailforge.config import LLMSettings, OpenClawSettings, Settings
from emailforge.llm import prompts
...
def test_prompt_override_prefers_user_dir_then_falls_back(tmp_path, monkeypatch):
    monkeypatch.setenv("EMAILFORGE_PROMPT_DIR", str(tmp_path))
...
def test_heavy_lift_on_when_configured(monkeypatch):
    monkeypatch.setenv("EMAILFORGE_HEAVY_KEY", "k-123")
    s = Settings(
...
def test_launcher_key_is_stable_and_0600(tmp_path, monkeypatch):
    ...
    monkeypatch.setattr("emailforge.paths.data_dir", lambda: tmp_path)
    from emailforge.ui import app as ui_app
