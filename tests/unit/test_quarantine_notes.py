...
from openclaw_email.db.store import open_store, register_sites, validate_site_id
...
def test_graph_refuses_quarantined_message(tmp_path):
    from openclaw_email.agent.graph import AgentGraph
...
def test_bridge_json_withholds_quarantined_bodies(tmp_path):
    from openclaw_email.bridge_cli import _message_json
...
def test_bridge_json_marks_untrusted_content(tmp_path):
    from openclaw_email.bridge_cli import UNTRUSTED_CLOSE, UNTRUSTED_OPEN, _message_json
...
def test_revise_refuses_quarantined_source(tmp_path):
    from openclaw_email.agent.revisions import revise_draft
...
def test_content_only_site_screens_before_any_model_sees_the_body():
    from openclaw_email.config import SiteConfig
    from openclaw_email.security.inbound_screening import assess_inbound
...
def test_safe_attachment_name_blocks_traversal():
    from openclaw_email.mail.imap_listener import _safe_attachment_name
...
def test_clean_subject_unfolds_header_linebreaks():
    from openclaw_email.ui.theme import clean_subject
